...
class RetryEngine:
    ...
    def record_attempt(self, strategy, success, error_type=None):
        with self._lock:
            ...
            # flow_mode arrives from config/session state and may be any string;
            # indexing a pre-seeded dict raised KeyError and killed the thread.
            if success:
                self._strategy_scores[strategy] = min(100, self._strategy_scores.get(strategy, 50) + 15)
            else:
                self._strategy_scores[strategy] = max(0, self._strategy_scores.get(strategy, 50) - 10)
