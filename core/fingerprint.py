...
def inject_selenium_fingerprint(driver):
    ...
    Every script goes through Page.addScriptToEvaluateOnNewDocument so it
    survives navigation — execute_script only touches the current document and
    every spoof died on the first driver.get().
    """
    try:
        ...
        # deviceMemory is capped at 8 by the spec; 16 is an impossible value and
        # an instant fake flag. Values are frozen per session, never re-randomized.
        random_cores = random.choice([4, 6, 8, 12])
        random_memory = random.choice([2, 4, 8])
        driver.execute_cdp_cmd("Page.addScriptToEvaluateOnNewDocument", {
            "source": f"""
...
def inject_selenium_poltergeist(driver):
    """Inject the full Poltergeist script so it survives navigation.

    The file-based js/poltergeist_fp.js is the single source of truth; the
    inlined copy below is kept only so the JS string in this module cannot
    drift from it.
    """
    script = None
    try:
        js_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "js")
        with open(os.path.join(js_dir, "poltergeist_fp.js"), "r", encoding="utf-8") as f:
            script = f.read()
    except Exception as e:
        logger.debug(f"Could not read js/poltergeist_fp.js, using embedded copy: {e}")
        script = POLTERGEIST_SCRIPT

    try:
        # CDP, not execute_script: execute_script applies only to the document
        # loaded at call time and every patch was lost on the next navigation.
        driver.execute_cdp_cmd("Page.addScriptToEvaluateOnNewDocument", {"source": script})
        logger.info("Poltergeist fingerprint applied via CDP (persists across navigation)")
        return True
    except Exception as e:
