...
def _create_single_account(index, total, engine, password, warmup_minutes,
    ...
    try:
        if engine == 'playwright':
            ...
            from core.selenium_runner import validate_birthday
            month, day, year = validate_birthday(Config.YOUR_BIRTHDAY)
    ...
    if success and proxy:
        ...
        # A UI/selector failure is not the proxy's fault; a fatal (−30) hit would
        # blacklist healthy proxies because of a stale selector.
...
def run_batch(num_accounts, max_threads=3, warmup_minutes=5,
    ...
    shared_password = Config.YOUR_PASSWORD

    if shared_password:
        logger.warning(
            "BATCH: all accounts will share one password — one leak compromises "
            "every account in this run. Leave YOUR_PASSWORD empty for per-account "
            "passwords."
        )
    else:
        try:
            with open("config/password.txt", "r", encoding="utf-8") as f:
                shared_password = f.read().strip()
        except FileNotFoundError:
            shared_password = ""

    def _password_for_account():
        # Unique password per account unless a shared one is explicitly configured.
        if shared_password:
    ...
    with ThreadPoolExecutor(max_workers=max_threads) as executor:
        ...
        for i in range(num_accounts):
            future = executor.submit(
                ...
                i, num_accounts, engine, _password_for_account(),
        ...
        for future in as_completed(futures):
            ...
            if on_result:
                try:
                    ...
                    logger.debug(f"on_result callback error: {cb_err}")

    duration = time.time() - start_time
    successes = sum(1 for r in results if r["success"])
    failures = len(results) - successes

    # Counter, not a literal 1 — the old dict comprehension mapped every error
    # type to 1 so the breakdown was always wrong.
