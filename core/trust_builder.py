...
def network_trust_check():
    ...
    try:
        ...
        # Fail CLOSED, not open: when we cannot determine the IP type, treat it as
        # a datacenter so the caller warns the user instead of proceeding as if
        # residential.
        logger.warning(f"IP info check failed: {e}")
        return {"ip": "Unknown", "city": "N/A", "country": "N/A",
                "is_datacenter": True, "error": str(e)}
...
def deep_trust_builder(driver, duration_minutes=5):
    ...
    while time.time() - start_time < target_duration:
        ...
        try:
            ...
            # Without this sleep, a dead driver makes every driver.get() throw and
            # the loop `continue`s with no delay — 100% CPU until duration elapses.
            logger.debug(f"Deep trust iteration failed: {e}")
            time.sleep(5)
...
def ultra_deep_trust_builder(driver, duration_minutes=10):
    ...
    for name, url in services:
        ...
        try:
            ...
            logger.debug(f"Ultra deep trust service tour failed for {url}: {e}")
            time.sleep(5)
    ...
    try:
        ...
        for query in random.sample(SEARCH_QUERIES, min(4, len(SEARCH_QUERIES))):
            ...
            try:
                ...
                logger.debug(f"Ultra deep trust search failed for {query!r}: {e}")
                time.sleep(5)
    except Exception:
        pass
...
def ghost_mode_prepare(driver, warmup_minutes=10):
    ...
    # Bound page loads: without this a hung page blocks every loop above
    # indefinitely, and the no-sleep retry paths burn a full CPU core.
    try:
    ...
    if net_info.get("is_datacenter"):
        logger.warning("Datacenter or unknown IP detected - residential proxy recommended")
