...
async def handle_phone_page(page, is_mobile=False, sms_api_available=False):
    ...
    if is_device_phone:
        ...
        escape_urls = [
            ...
            "https://accounts.google.com/lifecycle/steps/signup/name?continue=https%3A%2F%2Fmail.google.com&flowEntry=SignUp&flowName=GlifWebSignIn&hl=en&service=mail&theme=gli",
    ...
    # Strategies removed (v2):
    #   - back/forward navigation: Google re-renders the same challenge; the trick
    #     only burned time and generated non-human navigation bursts.
    #   - direct navigation to myaccount.google.com: an unauthenticated visit never
    #     completes signup, it just abandons the flow mid-way.
    #   - 10x Tab + Enter: does not bypass a server-side challenge and looks
    #     exactly like a bot to behavioural detection.
    #   - session reset + fresh signup: duplicated the should_restart path already
    #     handled by runners.py, and counted as a second signup attempt from the
    #     same flagged IP, which worsened the reputation.

    # Strategy 6: SMS API verification (if configured and enabled)
    ...
    logger.error("All phone bypass strategies exhausted — IP is likely flagged. "
                 "Use a residential/mobile proxy or enable Premium Mode with an SMS provider.")
...
async def handle_verification(page, is_mobile=False, use_sms_api=False, progress=None, account_task=None):
    ...
    if page_type == "phone":
        ...
        # Only a genuine escape to a fresh signup page should trigger a restart;
        # a plain "skip" already landed us past verification.
        should_restart = method in ("send_sms_escaped",)
