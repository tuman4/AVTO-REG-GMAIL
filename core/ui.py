...
def show_settings():
    ...
    table.add_row("Headless Mode", "Enabled" if Config.HEADLESS_MODE else "Disabled",
                  f"[{THEME['success']}]OK[/]" if Config.HEADLESS_MODE else f"[{THEME['muted']}]OFF[/]")
...
def show_accounts(accounts):
    ...
    console.print("  [3] Export to TXT (email:password)")
...
def _get_system_status():
    """Real measurements only. v1 displayed random PING/MEM values and
    re-randomized them on every render."""
    ...
    try:
        import subprocess
        latency_ms = subprocess.run(
            ["ping", "-n", "1", "www.google.com"],
            capture_output=True, text=True, timeout=5,
        )
        m = re.search(r"=(\d+)ms", latency_ms.stdout.replace(" ", ""))
        status["latency"] = f"{int(m.group(1))}ms" if m else "n/a"
    except Exception:
        status["latency"] = "n/a"
    try:
        import psutil
        status["memory"] = f"{psutil.virtual_memory().percent:.0f}%"
    except ImportError:
        status["memory"] = "n/a"
    return status
