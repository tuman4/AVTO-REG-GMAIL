...
Passwords are stored Fernet-encrypted via SecureVault, never in plaintext.
SQLite connections are closed after every call — sqlite3.connect used as a
context manager commits but does not close, which leaked one handle per
operation and deadlocked under the batch thread pool.
"""
import sqlite3
import os
import logging
...
class DatabaseManager:
    ...
    def _connect(self):
        """Open a connection that will actually be closed by the caller."""
        ...
        try:
            with closing(self._connect()) as conn:
                ...
                cursor.execute('''
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ''', (email, vault.encrypt(password), first_name, last_name, birthday, gender,
    ...
    def get_all_accounts(self):
        try:
            ...
            for acc in rows:
                acc['password'] = vault.decrypt(acc.get('password', ''))
            return rows
        except sqlite3.Error as e:
    ...
    def re_encrypt_plaintext_passwords(self):
        """One-time migration: encrypt any legacy plaintext rows in place."""
        ...
        try:
            with closing(self._connect()) as conn:
                ...
                for row_id, stored in rows:
                    if stored and not vault.is_encrypted(stored):
                        cursor.execute(
                            'UPDATE accounts SET password=? WHERE id=?',
                            (vault.encrypt(stored), row_id))
                        updated += 1
                conn.commit()
                if updated:
                    logger.info(f"SecureVault: encrypted {updated} legacy plaintext password(s)")
                return updated
        except sqlite3.Error as e:
            logger.error(f"Password re-encryption failed: {e}")
            return 0
    ...
    def run_migration(self, old_json_path="data/accounts.json", old_txt_path="data/accounts.txt"):
        ...
        if os.path.exists(old_json_path):
            try:
                with open(old_json_path, "r", encoding="utf-8") as f:
                    data = json.load(f)
                # A dict-shaped file iterates keys and migrates nothing; guard it.
        ...
        # Bring the existing DB rows under the vault too.
        self.re_encrypt_plaintext_passwords()
